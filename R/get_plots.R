...
get_base_plot <- function(dat, color=get_topr_colors(), show_legend=TRUE, legend_labels=NULL, legend_name=NULL,overview_plot=FALSE,legend_position="right"){
  ...
  for(i in seq_along(dat)){
    if(overview_plot){
     ...
     p1 <- p1+geom_point(data=dat[[i]], aes(x=POS, y=log10p, color=color), alpha=dat[[i]]$alpha, size=dat[[i]]$size,shape=dat[[i]]$shape)
  }
  ...
  if(length(dat) == 1 || !show_legend) { #no need to use a legend when there is only one phenotype on display
    ...
    p1 <- p1+scale_color_identity(guide = "legend", name=legend_name, breaks=color[seq_along(dat)], labels=legend_labels)+theme(legend.position = legend_position)
  }
  p1 <- rm_grids(p1)
  return(p1)
}
...
get_gene_plot <- function(df,label_size=3.5,xmin=xmin,xmax=xmax, show_gene_names=NULL){
  ...
  if(nrow(df$genes)>0){
    if(show_gene_names){
      p1 <- p1+geom_text(data=df$genes, aes(x=gene_start,y=as.numeric(y),label=gene_symbol),hjust=0, vjust=0, size=label_size)
    }
