...
regionplot <- function(df, ntop=3, annotate=NULL, xmin=0, size=2, shape=19, alpha=1,label_size=4, annotate_with="ID",
                    color=get_topr_colors(), axis_text_size=11,axis_title_size=12,title_text_size=13, show_genes=FALSE, show_overview=TRUE,
                       ...
                       xmax=NULL,ymin=NULL,ymax=NULL,protein_coding_only=FALSE,region=1000000,gene_region=100000,angle=0,legend_title_size=12,legend_text_size=12,
                       nudge_x=0.01,nudge_y=0.01, rsids=NULL, rsids_color="red",legend_name="Data:",legend_position="right",
                       chr=NULL,vline=NULL,show_gene_names=NULL,legend_labels=NULL,gene=NULL, title=NULL){
  ...
  if(! is.null(gene)){
    ...
    if(dim(gene_df)[1]==0){  stop(paste("Could not find gene ",gene)) }
     ...
     # if(is.null(sign_thresh_label_xpos)){
     #   sign_thresh_label_xpos <- xmin + ((xmax-xmin) * 0.0001)
     # }
    }
  }
  ...
  # get the annotation
  if(! is.null(annotate)){ top_snps <- get_annotation(dat, annotate=annotate, region=region) }
  # get the main plot
  main_plot <- get_base_plot(dat,color=color,legend_labels=legend_labels, legend_name = legend_name,legend_position = legend_position) %>% set_plot_text_sizes(axis_text_size=axis_text_size,axis_title_size = axis_title_size,legend_text_size=legend_text_size, legend_title_size=legend_title_size)

  main_plot <- main_plot+theme(axis.text.x=element_blank(), axis.title.x = element_blank(), axis.ticks.x=element_blank())
  main_plot <- main_plot %>%  add_annotation(plot_labels = top_snps,annotate_with=annotate_with,angle=angle,label_size = label_size, nudge_y = nudge_y, nudge_x = nudge_x) %>% add_vline(vline)
 ...
 main_plot <- main_plot %>% add_sign_thresh(sign_thresh = sign_thresh, sign_thresh_color = sign_thresh_color, using_ntop = using_ntop) %>%
  ...
  if( ! is.null(rsids)){
    main_plot <- main_plot %>% add_rsids(dat,rsids,rsids_color=rsids_color)
  }
 ...
 if((xmax-xmin < 1000001 || show_exons) & !show_genes){
  ...
  #get the genes in the region
    ...
    if(ngenes < max_genes){
      ...
      print("The number of genes can be reduced by either setting the protein_coding_only argument to TRUE (protein_coding_only=TRUE), or by reducing the gene_region parameter (e.g. gene_region=500)")
